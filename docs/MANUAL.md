...
Before any classes exist, the home screen also offers **Try a sample syllabus** — a short guided walkthrough. It creates a throwaway sample class and steps through the whole flow with a highlighted instruction bubble on each screen (tap **Got it** to advance, **Skip tour** to leave): the upload screen, the extracted-events preview, a simulated **Sync**, and the class editor. The parse is faked (no server call) and the simulated sync never touches Google Calendar. When the tour finishes — or you skip it — the sample class is deleted automatically, leaving no trace.
...
- A new class filed into a term inherits the term's length, and — if the term has "Auto-sync class meetings" on — starts with meeting sync enabled. The **Class Length** control in the class's meeting section (a week count, e.g. "10 weeks") overrides it for a shorter or longer class, or can be set to **open-ended**. The length sets the class's end date, which is what stops meeting recurrence and auto-switches the class to INACTIVE once it passes. (It only appears once the class has a schedule, a term, or an end date already set.)
...
- **Terms** — shows the active term and opens **Manage Terms**. A term folder is a named span (**Quarter** = 10 weeks, **Semester** = 16 weeks, or **Custom** with a weeks stepper) with a start date; the end date is derived from the length. A term can be typed a name or Plannr derives one from the start month ("Fall 2026"). Each term can also default its classes to have meeting sync on. See "Term Folders" below.
